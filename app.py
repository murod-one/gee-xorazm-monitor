...
# Sidebar (Yon panel) uchun dizayn uslublari
st.markdown("""
<style>
...
# Google Earth Engine ni initsializatsiya qilish (Xizmat hisobi orqali)
@st.cache_resource
def init_gee():
    try:
...
# Tumanlar ro'yxati va markaziy koordinatalari
DISTRICTS = [
...
# NDVI qiymatiga qarab meliorativ holatni tasniflash funksiyasi
def ndvi_class(val):
    if val is None:
        return "⚪ Aniqlanmadi"
    if val < 0.15:
...
# Yon paneldagi vidjetlar (Sidebar)
with st.sidebar:
...
if df_key not in st.session_state:
    with st.spinner("🛰️ GEE ma'lumotlari yuklanmoqda..."):
        try:
            # Sentinel-2 kolleksiyasini filtrlash va NDVI hisoblash
            s2 = (
            ...
            # Median (o'rtacha) mozaika tasvirni olish
            median_ndvi = s2.median()

            results = []
            # Har bir tanlangan tuman uchun hududiy o'rtacha NDVI ni hisoblash
            for dist in selected_districts:
                ...
                try:
                    val = median_ndvi.reduceRegion(
                        ...
                        geometry=point.buffer(5000), # Nuqta atrofida 5km bufer zona
                        scale=100,
                        maxPixels=1e9,
                    ).getInfo().get("NDVI")
                except Exception as e:
...
if USE_GEEMAP and median_ndvi is not None:
    try:
        ...
        # Xaritani Streamlit ichiga xavfsiz va toza HTML shaklida joylashtiramiz
...
if not USE_GEEMAP:
    st.error("❌ geemap yuklanmagan yoki xarita chizishda xatolik yuz berdi.")
...
# --- 5. YUKLAB OLISH BLOKI ---
st.markdown("## ⬇️ Ma'lumotlarni Yuklab Olish")
col_csv, col_json = st.columns(2)
col_csv.download_button(
    "📥 CSV formatda yuklash", data=df.to_csv(index=False),
    file_name=f"xorazm_ndvi_{start_str}_{end_str}.csv", mime="text/csv"
)
col_json.download_button(
    "📥 JSON formatda yuklash", data=df.to_json(orient="records", force_ascii=False),
    file_name=f"xorazm_ndvi_{start_str}_{end_str}.json", mime="application/json"
)

st.caption(f"📡 Tahlil yakunlandi. Ma'lumotlar manbasi: Sentinel-2 SR · GEE · {start_str} → {end_str}")
